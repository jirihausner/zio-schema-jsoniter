...
private[jsoniter] trait DecoderSpecs {
  ...
  final protected def assertDecodesToError[A](
    ...
    val stream = ZStream
      ...
      .tapError { message =>
        (Console.printLine(s"expected: $error") *>
  ...
  protected final val decoderSuite: Spec[Sized, DecodeError] = suite("decoding")(
    ...
    suite("fail")(
      test("of any cause") {
        assertDecodesToError(Schema.Fail[Exception]("Failed"), "null", "Failed")
      },
    ),
      ...
      suite("without discriminator")(
        ...
        suite("of case classes and case objects with more than 64 cases")(
          ...
          test("no discriminator key") {
            assertDecodesToError(Schema[BigEnum2], "{}", "missing subtype")
          },
          test("invalid case") {
            assertDecodesToError(
              Schema[BigEnum2],
      ...
      suite("with no discriminator")(
        ...
        test("fails on unrecognized case") {
          assertDecodesToError(
            Prompt.schema,
            """{"value":5}""",
            "none of the subtypes could decode the data",
          )
        },
      ),
